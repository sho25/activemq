...
begin_comment
comment|/**  * Test to determine if expired messages are being reaped if there is  * no active consumer connected to the broker.  */
end_comment
...
specifier|public
name|void
name|testExpiredMessagesOnTopic
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
decl_stmt|;
comment|// use a zero prefetch so messages don't go inflight
name|ActiveMQTopic
name|destination
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// should have a durable sub because it's a little tricky to get messages to expire in
comment|// non-durable subs.. with durable subs, we can just expire in the topic using the expire
comment|// period.. also.. durable sub has to be "inactive" for the expire checker to actually
comment|// expire the messages
...
argument_list|)
decl_stmt|;
comment|// not expired yet...
name|assertEquals
argument_list|(
literal|"Incorrect enqueue count"
argument_list|,
literal|3
argument_list|,
name|view
...
argument_list|)
expr_stmt|;
comment|// close consumer so topic thinks consumer is inactive
name|consumer
operator|.
name|close
argument_list|()
expr_stmt|;
comment|// Let the messages reach an expiry time
name|Thread
operator|.
name|sleep
argument_list|(
