...
name|protonTransport
operator|.
name|setOutboundFrameSizeLimit
argument_list|(
name|maxFrameSize
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// Ignore if older proton-j was injected.
block|}
block|}
name|this
operator|.
...
condition|)
block|{
comment|// caller treats 0 as no-work, ensure value is at least 1 as there was a deadline
name|rescheduleAt
operator|=
name|Math
operator|.
...
name|warn
argument_list|(
literal|"Error finding subscription for: "
operator|+
name|consumerInfo
operator|+
literal|": "
...
condition|)
block|{
comment|// monitor treats<= 0 as no work, ensure value is at least 1 as there was a deadline
name|long
name|delay
init|=
name|Math
...
name|trace
argument_list|(
literal|"Connection keep-alive processing starts in: {}"
argument_list|,
name|delay
argument_list|)
expr_stmt|;
