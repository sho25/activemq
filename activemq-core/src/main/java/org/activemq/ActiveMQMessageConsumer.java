...
condition|)
block|{
comment|// Validate that the selector
operator|new
name|SelectorParser
argument_list|()
operator|.
...
parameter_list|()
block|{
comment|// TODO Add ActiveMQTopicSubscriber
return|return
literal|false
return|;
comment|// return this instanceof ActiveMQTopicSubscriber&& consumerName !=
comment|// null&& consumerName.length()> 0;
block|}
...
return|;
block|}
comment|/**      * Receives the next message that arrives within the specified timeout      * interval.      *<P>      * This call blocks until a message arrives, the timeout expires, or this      * message consumer is closed. A<CODE>timeout</CODE> of zero never      * expires, and the call blocks indefinitely.      *       * @param timeout      *            the timeout value (in milliseconds)      * @return the next message produced for this message consumer, or null if      *         the timeout expires or this message consumer is concurrently      *         closed      */
specifier|public
name|Message
name|receive
parameter_list|(
...
name|warn
argument_list|(
literal|"could not process message: "
operator|+
name|md
argument_list|,
name|e
