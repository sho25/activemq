...
name|trace
argument_list|(
literal|"Syncing file {} on rotate"
argument_list|,
name|dataFile
operator|.
name|getFile
...
expr_stmt|;
block|}
comment|// append 'unset', zero length next batch so read can always find eof
name|buff
operator|.
name|write
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// revert sync batch increment such that next write is contiguous
if|if
condition|(
name|syncBatch
argument_list|(
...
block|}
block|}
comment|// async writes will already be in the index so reuse is not an option
specifier|private
name|boolean
name|syncBatch
parameter_list|(
