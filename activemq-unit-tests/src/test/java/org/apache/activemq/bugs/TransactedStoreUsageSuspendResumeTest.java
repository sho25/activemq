...
name|info
argument_list|(
literal|"Giving up - not all received on time..."
argument_list|)
expr_stmt|;
name|LOG
operator|.
...
name|info
argument_list|(
literal|"Consumer remaining to receive: "
operator|+
name|messagesReceivedCountDown
operator|.
name|getCount
...
expr_stmt|;
block|}
comment|// issue with gc and linear store usage
comment|// some daylight in needed between retainQ and regularQ to free up the store
comment|// log4j.logger.org.apache.activemq.store.kahadb.MessageDatabase=TRACE
name|Destination
name|shortRetainQueue
init|=
name|session
operator|.
name|createQueue
...
name|info
argument_list|(
literal|"Done with retain q. Mem Usage: "
operator|+
name|broker
operator|.
name|getSystemUsage
...
name|info
argument_list|(
literal|"Done with retain q. Store Usage: "
operator|+
name|broker
operator|.
name|getSystemUsage
