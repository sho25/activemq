...
literal|null
decl_stmt|;
comment|// use existing object to sync orderIndexUpdates that can be reassigned
synchronized|synchronized
init|(
name|sendLock
init|)
...
argument_list|()
expr_stmt|;
comment|// talking all the ordered work means that earlier
comment|// and later threads do nothing.
comment|// this avoids contention/race on the sendLock that
comment|// guards the actual work.
block|}
block|}
comment|// do the ordered work
if|if
condition|(
name|orderedWork
operator|!=
