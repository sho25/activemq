...
condition|)
block|{
comment|//add the subscriptions to a list for recovering pending sizes below
name|subscriptionKeys
operator|.
name|add
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//recover just the count here as that is fast
name|statistics
operator|.
name|getMessageCount
argument_list|(
...
block|}
block|}
comment|//Recover the message sizes for each subscription by iterating only 1 time over the order index
comment|//to speed up recovery
specifier|final
name|Map
argument_list|<
name|String
