...
argument_list|()
expr_stmt|;
comment|// introduce a second broker/bridge on remote that should not get any messages because of networkTtl=1
comment|// local<-> remote<-> secondRemote
name|createSecondRemoteBroker
argument_list|()
expr_stmt|;
name|config
operator|=
...
name|remoteTransport
operator|=
name|createSecondRemoteTransport
argument_list|()
expr_stmt|;
comment|// Create a network bridge between the two brokers
name|bridge
...
name|assertTrue
argument_list|(
literal|"JMSActiveMQBrokerPath property present and correct"
argument_list|,
operator|(
operator|(
name|ActiveMQMessage
...
argument_list|)
expr_stmt|;
comment|// try and pull the messages from remote, should be denied b/c on networkTtl
name|LOG
operator|.
name|info
argument_list|(
literal|"creating demand on second remote..."
argument_list|)
expr_stmt|;
name|StubConnection
name|connection3
init|=
name|createSecondRemoteConnection
argument_list|()
decl_stmt|;
name|ConnectionInfo
...
name|error
argument_list|(
literal|"got message on Second remote: "
operator|+
name|messageExceedingTtl
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Closing consumer on remote"
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// also close connection etc.. so messages get dropped from the local consumer  q
name|connection2
operator|.
name|send
argument_list|(
...
name|assertTrue
argument_list|(
literal|"can see broker path property"
argument_list|,
operator|(
call|(
name|String
...
name|info
argument_list|(
literal|"Messages now stuck on remote"
argument_list|)
expr_stmt|;
comment|// receive again on the origin broker
name|ConsumerInfo
name|consumerInfo1
init|=
name|createConsumerInfo
...
name|assertNotNull
argument_list|(
literal|"Expect to get a replay as remote consumer is gone"
argument_list|,
name|message1
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"acked one message on origin, waiting for all messages to percolate back"
argument_list|)
expr_stmt|;
name|Wait
operator|.
...
name|info
argument_list|(
literal|"checking for messages on remote again"
argument_list|)
expr_stmt|;
comment|// messages won't migrate back again till consumer closes
name|connection2
operator|=
name|createRemoteConnection
argument_list|()
...
name|assertNull
argument_list|(
literal|"Messages have migrated back: "
operator|+
name|message1
argument_list|,
name|message1
argument_list|)
expr_stmt|;
comment|// Consume the last 4 messages from the local broker and ack them just
...
argument_list|)
expr_stmt|;
comment|// verify all messages consumed
name|Wait
operator|.
name|waitFor
argument_list|(
...
name|secondRemoteConnector
operator|=
name|createSecondRemoteConnector
argument_list|()
expr_stmt|;
name|secondRemoteBroker
operator|.
