...
name|waitForMessagesToBeConsumed
argument_list|(
name|brokerA
argument_list|,
literal|"Consumer.B.VirtualTopic.tempTopic"
...
specifier|public
name|void
name|testSelectorNoMatchInCache
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
expr_stmt|;
comment|// have the cache ignoreWildcardSelectors
specifier|final
name|BrokerService
name|brokerA
init|=
...
expr_stmt|;
block|}
comment|/**      * Typically used before asserts to give producers and consumers some time to finish their tasks      * before the final state is tested.      *      * @param broker BrokerService on which the destinations are looked up      * @param destinationName      * @param topic true if the destination is a Topic, false if it is a Queue      * @param numEnqueueMsgs expected number of enqueued messages in the destination      * @param numDequeueMsgs expected number of dequeued messages in the destination      * @param waitTime number of milliseconds to wait for completion      * @throws Exception      */
specifier|private
name|void
name|waitForMessagesToBeConsumed
parameter_list|(
