...
argument_list|()
decl_stmt|;
comment|// TODO - we could remove this if we have a way to disable BooleanStream on
comment|// certain types of message
if|if
condition|(
name|type
operator|==
...
block|{
return|return
name|doUnmarshalPartialCommand
argument_list|(
name|dataType
argument_list|,
name|dis
...
expr_stmt|;
block|}
comment|// Partial command marshalling
comment|//
comment|// TODO - remove if we can figure out a clean way to disable BooleanStream in OpenWire on commands
comment|// with no optional values (partial commands only have a mandatory byte[])
comment|//
specifier|protected
name|void
name|marshalPartialCommand
...
name|IOException
block|{
comment|// size of entire command is already read
name|PartialCommand
name|answer
init|=
literal|null
