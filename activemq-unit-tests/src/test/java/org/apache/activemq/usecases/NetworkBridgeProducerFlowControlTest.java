...
specifier|public
name|void
name|testSendFailIfNoSpaceReverseDoesNotBlockQueueNetwork
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// Set a policy on the local broker that limits the maximum size of the
comment|// slow shared queue.
name|PolicyEntry
name|policyEntry
init|=
...
argument_list|()
expr_stmt|;
comment|// Start two asynchronous consumers on the local broker, one for each
comment|// of the two shared queues, and keep track of how long it takes for
comment|// each of the consumers to receive all the messages.
specifier|final
name|CountDownLatch
...
expr_stmt|;
block|}
comment|/**      * create a duplex network bridge from broker0 to broker1      * add a topic consumer on broker0      * set the setSendFailIfNoSpace() on the local broker.      * create a SimpleDiscoveryAgent impl that tracks a network reconnect      *      * producer connects to broker1 and messages should be sent across the network to broker0      *      * Ensure broker0 will not send the  javax.jms.ResourceAllocationException (when broker0 runs out of space).      * If the javax.jms.ResourceAllocationException is sent across the wire it will force the network connector      * to shutdown      *      *      * @throws Exception      */
specifier|public
name|void
name|testDuplexSendFailIfNoSpaceDoesNotBlockNetwork
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
operator|+
literal|".duplexTest?consumer.prefetchSize=1"
argument_list|)
decl_stmt|;
specifier|final
name|int
...
argument_list|)
expr_stmt|;
comment|// Create a duplex network bridge from the local broker to the remote broker
comment|// create a SimpleDiscoveryAgent impl that tracks a reconnect
name|DiscoveryNetworkConnector
name|discoveryNetworkConnector
init|=
operator|(
...
argument_list|)
expr_stmt|;
name|DummySimpleDiscoveryAgent
name|dummySimpleDiscoveryAgent
init|=
operator|new
name|DummySimpleDiscoveryAgent
...
argument_list|)
decl_stmt|;
comment|//createConsumer("broker0", fastDestination, fastConsumerLatch);
name|MessageConsumer
name|consumer
init|=
name|createConsumer
...
argument_list|)
expr_stmt|;
comment|//wait for 5 seconds for the consumer to complete
name|consumerLatch
operator|.
name|await
argument_list|(
...
name|assertFalse
argument_list|(
literal|"dummySimpleDiscoveryAgent.serviceFail has been invoked - should not have been"
argument_list|,
name|dummySimpleDiscoveryAgent
operator|.
name|isServiceFailed
argument_list|)
expr_stmt|;
block|}
comment|/**      * When the network connector fails it records the failure and delegates to real SimpleDiscoveryAgent      */
class|class
name|DummySimpleDiscoveryAgent
extends|extends
name|SimpleDiscoveryAgent
...
name|IOException
block|{
comment|//should never get in here
name|LOG
operator|.
name|info
argument_list|(
literal|"!!!!! DummySimpleDiscoveryAgent.serviceFailed() invoked with event:"
operator|+
name|devent
operator|+
literal|"!!!!!!"
