...
begin_comment
comment|/**  * This PacketAggregatingSynchChannel can be used when the client is sending a  * 'record' style packet down the channel stack and needs receiving end to  * receive the same 'record' packets.  *   * This is very usefull since in general, a channel does not garantee that a  * Packet that is sent down will not be fragmented or combined with other Packet  * objects.  *   * This {@see org.apache.activeio.SynchChannel} adds a 4 byte header  * to each packet that is sent down.  *   * @version $Revision$  */
end_comment
