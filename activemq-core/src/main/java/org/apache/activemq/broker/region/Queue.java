...
block|}
block|}
comment|// do recovery dispatch only if it is a browser subscription
if|if
condition|(
name|sub
operator|instanceof
...
block|}
block|}
comment|//if it is a last consumer (and not a browser) dispatch all pagedIn messages
if|if
condition|(
name|consumers
operator|.
...
name|error
argument_list|(
literal|"Problem retrieving message in browse() "
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// Log showing message dispatching
if|if
condition|(
name|LOG
operator|.
...
argument_list|()
operator|+
literal|" - Recovery - Message pushed '"
operator|+
name|node
operator|.
name|hashCode
