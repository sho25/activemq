...
decl_stmt|;
name|long
name|previousDequeueCount
init|=
operator|-
literal|1
decl_stmt|;
name|long
name|previousDequeueCountRepeated
init|=
literal|1L
decl_stmt|;
name|long
...
condition|)
block|{
comment|// Break the infinite loop in case the removal fails
comment|// 3 times in a row -> error is fatal and not transient.
name|LOG
operator|.
name|error
argument_list|(
literal|"Aborted purge operation after attempting to delete messages failed 3 times in a row (to avoid endless looping)"
argument_list|)
expr_stmt|;
throw|throw
operator|new
name|RuntimeException
argument_list|(
literal|"Purge operation failed to delete messages failed 3 times in a row (to avoid endless looping)"
argument_list|)
throw|;
block|}
block|}
