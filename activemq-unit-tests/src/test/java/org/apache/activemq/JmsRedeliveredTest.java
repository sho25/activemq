...
specifier|public
name|void
name|testNoReceiveConsumerDisconnectDoesIncrementRedelivery
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
name|Connection
name|keepBrokerAliveConnection
init|=
name|createConnection
argument_list|()
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// whack the connection - like a rebalance or tcp drop - consumer does not get to communicate
comment|// a close and delivered sequence info to broker. So broker is in the dark and must increment
comment|// redelivery to be safe
operator|(
operator|(
...
specifier|public
name|void
name|testNoReceiveConsumerAbortDoesNotIncrementRedelivery
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// on abort via something like slowConsumerPolicy
name|ConsumerControl
name|consumerControl
init|=
operator|new
