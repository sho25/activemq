...
name|transport
decl_stmt|;
comment|/*         if (tcpTransport.isTrace()) {             transport = new TransportLogger(transport);         }          if (tcpTransport.getMaxInactivityDuration()> 0) {             transport = new InactivityMonitor(transport, tcpTransport.getMaxInactivityDuration());         }          transport = new ResponseCorrelator(transport);         */
return|return
name|transport
return|;
block|}
