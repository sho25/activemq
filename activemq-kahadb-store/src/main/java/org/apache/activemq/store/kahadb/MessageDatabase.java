...
try|try
block|{
comment|// run before with the index lock so that queue can order cursor updates with index updates
if|if
condition|(
name|before
operator|!=
...
comment|// store a DUP message. Bad BOY! Don't do it, and log a warning.
name|LOG
operator|.
name|warn
argument_list|(
