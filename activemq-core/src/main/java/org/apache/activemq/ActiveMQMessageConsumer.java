...
condition|)
block|{
comment|// second call from rollback, nothing more to do
comment|// REVISIT - should beforeEnd be called again by transaction context?
name|rollbackInitiated
operator|=
literal|false
expr_stmt|;
...
name|debug
argument_list|(
literal|"previously delivered message has not been replayed in transaction, id: "
operator|+
name|entry
operator|.
name|getKey
...
name|message
init|=
literal|"rolling back transaction post failover recovery. "
operator|+
name|numberNotReplayed
operator|+
...
block|{
comment|// existing transaction gone but still a duplicate!, lets mark as poison ftm,
comment|// possibly could allow redelivery..
name|needsPoisonAck
operator|=
literal|true
expr_stmt|;
...
name|warn
argument_list|(
literal|"acking as poison, duplicate transacted delivery but no recovering transaction for: "
operator|+
name|md
argument_list|)
expr_stmt|;
