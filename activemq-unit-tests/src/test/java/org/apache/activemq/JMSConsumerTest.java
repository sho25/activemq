...
specifier|public
name|void
name|testMessageListenerWithConsumerCanBeStoppedConcurently
parameter_list|()
throws|throws
name|Exception
block|{
...
condition|)
block|{
comment|// close in a separate thread is ok by jms
name|consumer
operator|.
name|close
argument_list|()
...
name|error
argument_list|(
literal|"Exception on close or ack:"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// ack and close eventually in separate thread
name|executor
operator|.
name|execute
argument_list|(
...
specifier|public
name|void
name|initCombosForTestPassMessageListenerIntoCreateConsumer
parameter_list|()
block|{
name|addCombinationValues
argument_list|(
...
specifier|public
name|void
name|testPassMessageListenerIntoCreateConsumer
parameter_list|()
throws|throws
name|Exception
block|{
...
specifier|public
name|void
name|initCombosForTestMessageListenerOnMessageCloseUnackedWithPrefetch1StayInQueue
parameter_list|()
block|{
name|addCombinationValues
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// order is not guaranteed as the connection is started before the listener is set.
comment|// assertEquals("" + counter.get(), tm.getText());
name|counter
operator|.
name|incrementAndGet
argument_list|()
...
specifier|public
name|void
name|initCombosForTestMessageListenerAutoAckOnCloseWithPrefetch1
parameter_list|()
block|{
name|addCombinationValues
argument_list|(
...
specifier|public
name|void
name|testMessageListenerAutoAckOnCloseWithPrefetch1
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// close from onMessage with Auto_ack will ack
comment|// Make sure only 4 messages were delivered.
name|assertEquals
argument_list|(
literal|4
...
argument_list|)
expr_stmt|;
comment|// Make sure only 4 message are delivered.
for|for
control|(
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|// Close out the consumer.. no other messages should be left on the queue.
name|consumer
operator|.
name|close
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// install another consumer while message dispatch is unacked/uncommitted
name|Session
name|redispatchSession
init|=
name|connection
operator|.
name|createSession
...
argument_list|)
decl_stmt|;
comment|// no commit so will auto rollback and get re-dispatched to redisptachConsumer
name|session
operator|.
name|close
argument_list|()
...
name|fail
argument_list|(
literal|"Expect exception on ack after close - consumer gone so message available again"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
specifier|public
name|void
name|initCombosForTestAckOfExpired
parameter_list|()
block|{
name|addCombinationValues
argument_list|(
...
name|ActiveMQConnectionFactory
argument_list|(
literal|"vm://localhost?jms.prefetchPolicy.all=4&jms.sendAcksAsync=false"
argument_list|)
decl_stmt|;
name|connection
operator|=
...
expr_stmt|;
block|}
comment|// let first bunch in queue expire
name|Thread
operator|.
name|sleep
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// force an ack when there are expired messages
name|amqConsumer
operator|.
name|acknowledge
argument_list|()
...
name|assertEquals
argument_list|(
literal|"consumer has expiredMessages"
argument_list|,
name|count
argument_list|,
name|amqConsumer
...
name|domain
operator|+
literal|":type=Broker,brokerName=localhost,destinationType=Queue,destinationName=test"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|domain
operator|+
literal|":type=Broker,brokerName=localhost,destinationType=Topic,destinationName=test"
argument_list|)
expr_stmt|;
block|}
return|return
