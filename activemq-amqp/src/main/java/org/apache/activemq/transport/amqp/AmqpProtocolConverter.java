...
name|context
decl_stmt|;
comment|// change consumer prefetch if it's not been already set using
comment|// transport connector property or consumer preference
if|if
condition|(
name|consumerContext
operator|.
...
block|{
return|return
literal|"ProducerContext { producerId = "
operator|+
name|producerId
operator|+
literal|", destination = "
...
condition|)
block|{
comment|// use the value configured on the transport connector
comment|// this value will not be changed to the consumer's preference
name|consumerInfo
operator|.
...
condition|)
block|{
comment|// set the prefetch to the value of the remote credit
comment|// and ignore the later changes
name|consumerInfo
operator|.
name|setPrefetchSize
argument_list|(
