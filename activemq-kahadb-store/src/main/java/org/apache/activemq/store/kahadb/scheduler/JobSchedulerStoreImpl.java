...
block|}
block|}
comment|/**      * Removes multiple references for the Journal log file indicated in the given Location map.      *      * The references are used to track which log files cannot be GC'd.  When the reference count      * on a log file reaches zero the file id is removed from the tracker and the log will be      * removed on the next check point update.      *      * @param tx      *      The TX under which the update is to be performed.      * @param decrementsByFileIds      *      Map indicating how many decrements per fileId.      *      * @throws IOException if an error occurs while updating the journal references table.      */
specifier|protected
name|void
name|decrementJournalCount
parameter_list|(
